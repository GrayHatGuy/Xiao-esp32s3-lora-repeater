...
Status: **both firmwares build green; PARTIALLY bench-tested on hardware
2026-06-07.** Implements tracked task #1 (RX-priority route-queue + CAD +
hash-dedup) and task #2 (full-mesh airtime throttle). Full design + as-built map +
honest bench status in [`ROUTING-REDESIGN.md`](ROUTING-REDESIGN.md) §9. Branch off
`T_LORA_QUAD` @ `3b06f3c`; merge is a later owner decision.

**Bench result (XIAO + T-Lora-Dual, all sub-GHz) — PARTIAL:** the host **SX1262
side (R1/R2)** is validated end-to-end — MC↔MT both directions, text + telemetry
(env/position) + NodeInfo, content-hash dedup dropping echoes, clean far-side
bodies. On the **LR1121 co-proc (R3/R4, over UART):** R4 RX+TX and R3 RX were
demonstrated **over the air at CLOSE RANGE only** (−11…−49 dBm); **R3 TX was not
isolated** (co-channel with R1), and **weak/distant LR1121 RX was never tested.**
So the Core1121-style LR11x0 RX-sensitivity question (§9) is **OPEN, not closed**,
and the Semtech `lr11xx_driver` port stays a contingency. (An earlier draft of this
entry claimed "LR1121 RX/TX work / deficit did not materialise" — that over-claimed
