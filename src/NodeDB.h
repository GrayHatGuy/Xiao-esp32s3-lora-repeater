...
// consulted by the identity layer so MT -> MC bridged messages can carry the
// actual sender's short_name ("Alice@MT: ...") instead of an anonymous prefix.
//
// Storage model:
...
// concurrently. This matters from the RX-priority pipeline onward, where more
