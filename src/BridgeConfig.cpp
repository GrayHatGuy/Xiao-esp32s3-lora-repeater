...
// BRIDGE_* / LORA_RADIO*_SYNC_WORD macros in platformio.ini.
...
// Schema v3 — per-radio channel name + key.
struct PersistedV3 {
    uint8_t  version;
    uint8_t  configured;
    uint8_t  positionEnabled;
...
void begin() {
    ...
    if (blobSize == sizeof(PersistedV3)) {
        PersistedV3 tmp;
        size_t got = prefs.getBytes(NVS_KEY_BLOB, &tmp, sizeof(tmp));
        if (got == sizeof(PersistedV3) && tmp.version == 3) {
            s_cfg = tmp;
            terminateAll();
            Serial.printf("[BridgeConfig] loaded v3 blob from NVS (configured=%u)\n",
        ...
        if (got == sizeof(PersistedV2) && v2.version == 2) {
            ...
            Serial.printf("[BridgeConfig] migrated v2 blob -> v3 (configured=%u)\n",
                          (unsigned)s_cfg.configured);
        } else {
