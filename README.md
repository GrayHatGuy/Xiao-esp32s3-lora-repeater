...
**What's new in each release:** see [`CHANGELOG.md`](CHANGELOG.md) for the full per-version changelog — the **v8.4 LoRaWAN ABP uplink encoder** is the latest addition (this `dev-ABP-lorawan` branch, targeting the v8.4 release), on top of the published **[v8.3.1 release](https://github.com/GrayHatGuy/Xiao-esp32s3-lora-repeater/releases/tag/v8.3.1)** (current `main` — the Radio-2 V1.0/V1.1 fix, also folded into this branch). Design + bench docs: [`ABP-LORAWAN-SPEC.md`](ABP-LORAWAN-SPEC.md), [`V8.3-SPEC.md`](V8.3-SPEC.md), [`V8.2-SPEC.md`](V8.2-SPEC.md), [`BENCH-v8.3.md`](BENCH-v8.3.md).
...
> **Advanced — v8.4 LoRaWAN ABP uplink encoder (opt-in, source build).** Emitting valid LoRaWAN is *keyed*, so this is configured by build flags rather than the captive portal (a portal section + per-source key/FCnt persistence are roadmap). Set `BRIDGE_LW_ENCODE=1` plus a `BRIDGE_LW_ENC_DEVADDR` and the `BRIDGE_LW_ENC_NWKSKEY` / `BRIDGE_LW_ENC_APPSKEY` session keys (see [Build flags](#build-flags--compile-time-configuration) below), provision the matching ABP device in ChirpStack (MAC 1.0.x, ABP, Class A, ADR off), and a `0x34` destination radio will re-emit your decoded mesh (MT/MC) traffic as valid ABP uplinks over RF for a gateway to forward. The encoder is crypto-verified; **on-air ChirpStack ingestion is the open bench item**, and a raw-LoRa / Custom (weather-station) source is a later phase. Full design: [`ABP-LORAWAN-SPEC.md`](ABP-LORAWAN-SPEC.md).
...
- `BRIDGE_LW_ENCODE` (0) — master switch. When `1` **and** both session keys parse, `MT/MC → LoRaWAN` becomes a keyed **ABP uplink encode** (RF re-emit / delivery model B1) instead of the keyless `no-lw-encoder` drop; otherwise the keyless tap is unchanged (do-no-harm). *(On-air ChirpStack ingestion is the open bench item; a raw-LoRa/Custom source is P3.)*
...
- *Build-flag config + an in-RAM FCnt for now; per-source credentials, a captive-portal section and an NVS-persisted 32-bit FCnt are roadmap (P2). See [`ABP-LORAWAN-SPEC.md`](ABP-LORAWAN-SPEC.md).*
...
| **LoRaWAN (LW)** | `0x34` | **Keyless** — cleartext MAC header only (MType / DevAddr / FCtrl / FCnt / FPort / len; JoinEUI/DevEUI on joins); no key, no payload decrypt | **Capture** log + **metadata summary** to MT/MC (`BRIDGE_LW_SUMMARY_TO_MESH`); **LW↔LW transparent raw relay / dedup-bounded flood** (`BRIDGE_LW_RELAY`). **`MT/MC → LW`: keyed ABP uplink encoder (v8.4, opt-in)** — with ABP credentials set (`BRIDGE_LW_ENCODE` + keys), transcodes to a valid LoRaWAN ABP uplink (CMAC MIC + AES-CTR FRMPayload + monotonic FCnt) and RF-re-emits for a gateway → ChirpStack (crypto-verified; on-air bench pending); default (no keys) stays the keyless `no-lw-encoder` drop | keyless tap: summaries carry the bridge's own MT/MC id. ABP encode: each uplink is sent under a bridge-held ABP device identity (DevAddr) |
...
- [x] ~~**v8.4: LoRaWAN ABP uplink encoder (keyed).**~~ — **implemented** on `dev-ABP-lorawan` (build-green + crypto-verified; on-air ChirpStack bench pending); specced in [`ABP-LORAWAN-SPEC.md`](ABP-LORAWAN-SPEC.md). A self-contained **RFC 4493 AES-CMAC** (the prebuilt esp32s3 mbedTLS ships CMAC disabled) + `encodeLoRaWANUplink()` mint valid LoRaWAN 1.0.x **ABP uplinks** (AES-CTR FRMPayload, CMAC MIC, monotonic FCnt, ADR off, Unconfirmed). The keyless `no-lw-encoder` drop becomes a keyed transcode → `g_routeQ` **RF re-emit** (delivery model B1) when ABP credentials are configured — opt-in via `BRIDGE_LW_ENCODE` + keys, so a stock build keeps v8.3's keyless behavior. Crypto cross-checked against an independent CMAC (RFC 4493 vectors + a MIC-valid, round-tripping frame). Per-source credentials, a captive-portal section and an NVS-persisted 32-bit FCnt are the next phase (P2).
...
- [ ] **LoRaWAN ABP encode — next phases + key-based decode + OTAA.** The keyless tap shipped in v8.3 and the **ABP uplink *encode*** (`MT/MC → LoRaWAN`) landed in **v8.4** (above). Remaining: per-source credentials + captive-portal config + NVS-persisted FCnt (P2), the universal source→FPort/payload mapping + a sample ChirpStack codec (P3), regional dwell/duty enforcement (P4), and the optional ABP/OTAA **decode** of your own fleet (→ MT/MC) — see [`ABP-LORAWAN-SPEC.md`](ABP-LORAWAN-SPEC.md). Content bridging without keys stays architecturally precluded (analysis in [`V8.2-SPEC.md`](V8.2-SPEC.md) §14).
