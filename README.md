...
- [ ] **v8: vanilla firmware — full portal config.** Specced in [`V8-SPEC.md`](V8-SPEC.md) (status: ready, all open decisions confirmed; no code yet). Ship a single distributable `.bin` configured *entirely* through the captive portal — no build flags required. Adds a per-radio protocol picker (Meshtastic / MeshCore / Reticulum / Custom / None), a global region selector (US, EU_868, EU_433, ANZ, CN, JP, IN, KR, RU + Custom/Other), Tier 2 Meshtastic channel-slot frequency computation with an editable override, null defaults (a no-flag image first-boots straight into the portal), and MAC-derived identity/SSID. `BridgeConfig` schema v3→v4. `platformio.ini` `LORA_RADIO*` flags become optional first-boot pre-seeding. This reverses the "protocol/RF is config-time only" decision for the vanilla-bin use case — the Custom RF path sits behind an explicit warning banner.
- [ ] **Compile-time validation of build-flag config.** `#error` guards that reject an invalid `LORA_RADIO*` set at build time (sync words, then SF/CR/BW/region sanity). Complements the v8 portal-side runtime validation — build-time checks defaults, portal checks portal-entered values.
