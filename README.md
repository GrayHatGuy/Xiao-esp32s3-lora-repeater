...
- [ ] **Sub-GHz ↔ 2.4 GHz LoRa cross-band bridging — Phase 1 ON HOLD pending Seeed clarification.** The current build talks the SX1262's native sub-GHz ranges (902-928 MHz US ISM, 868 MHz EU, etc.). A long-horizon goal is to bridge those to 2.4 GHz LoRa networks (e.g. Meshtastic's 2.4 GHz preset) on the worldwide-licence-free **2.4 GHz ISM band** — the headline feature that makes this milestone worth doing.
  ...
  > **⚠️ Phase 1 status (2026-05-26): on hold.** A first attempt at this milestone using the **Seeed Wio-LR1121 module (SKU 113991415)** as Radio 2 reached a hard block: TX works end-to-end (other LoRa receivers see the bridge's transmissions), but RX produces zero `RX_DONE` events for any over-the-air traffic — even with a transmitting MT phone's antenna physically pressed against the LR1121's antenna port. We exhaustively swept all 8 possible `MODE_RX` states of DIO5/DIO6/DIO7 on the LR1121's RF switch, with no change in behavior. Conclusion: the LR1121's antenna → switch → LNA chain is electrically disconnected on this module in our setup. Two independently-sourced units behave identically. A comprehensive bug report has been **drafted and is ready to send** to Seeed engineering (see [`SEEED_SUPPORT_INQUIRY.md`](https://github.com/GrayHatGuy/Xiao-esp32s3-lora-repeater/blob/lr1121-phase1/SEEED_SUPPORT_INQUIRY.md) and [`SEEED_RECOMMENDATIONS.md`](https://github.com/GrayHatGuy/Xiao-esp32s3-lora-repeater/blob/lr1121-phase1/SEEED_RECOMMENDATIONS.md) on the [`lr1121-phase1`](https://github.com/GrayHatGuy/Xiao-esp32s3-lora-repeater/tree/lr1121-phase1) branch) — **vendor escalation is being held pending completion of an in-progress chip-init audit experiment** (see [`LR1121-RX-INIT-AUDIT.md`](https://github.com/GrayHatGuy/Xiao-esp32s3-lora-repeater/blob/lr1121-phase1/LR1121-RX-INIT-AUDIT.md)) which may resolve the RX block locally via the chip's `SetRssiCalibration` (the top-candidate root cause per Semtech LR1121 UM v2.2 §7.2.15: the chip ships calibrated for the LR1121 EVK band, not for the Wio-LR1121's matching network — incorrect per-PCB calibration causes the chip's AGC to pick the wrong LNA gain, producing exactly the "missed packets / decreased reception" symptom we observe). The complete investigation (diagnostic instrumentation, RF switch table integration, 12-combination exhaustive RFSWx sweep, all serial-log evidence) lives on that branch and at the snapshot tag [`lr1121-bringup-2026-05-26`](https://github.com/GrayHatGuy/Xiao-esp32s3-lora-repeater/tree/lr1121-bringup-2026-05-26). **`main` continues to ship the Phase 0 dual-SX1262 release (v8.1).** Phase 1 will resume either via the local bench-DOE fix, via Seeed engineering's reply (if escalation becomes necessary), or via alternate LR1121 carrier hardware confirming whether the issue is module-specific.
